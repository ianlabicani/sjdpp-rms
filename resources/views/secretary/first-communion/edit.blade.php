...
<div class="pt-16 min-h-screen bg-gray-50" x-data="communionForm()">
    <div class="max-w-3xl mx-auto px-3 sm:px-6 lg:px-8 py-6 sm:py-8">
        ...
        <div class="mb-6">
            ...
            <h1 class="text-2xl sm:text-3xl font-bold text-gray-900 mt-2">Edit First Communion Record</h1>
        </div>

        <!-- Form -->
        <div class="bg-white rounded-lg shadow-md p-4 sm:p-6">
            <form action="{{ route('secretary.first-communion.update', $firstCommunion) }}"
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
                        ...
                        <div>
                            ...
                            <input type="number"
                                   name="year"
                                   id="year"
                                   value="{{ old('year', $firstCommunion->year) }}"
                                   min="1900"
                                   max="{{ now()->year }}"
                                   required
                        ...
                        <div>
                            ...
                            <select name="month"
                                ...
                                @for($i = 1; $i <= 12; $i++)
                                    <option value="{{ $i }}" {{ old('month', $firstCommunion->month) == $i ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <input type="number"
                                   name="day"
                                   id="day"
                                   value="{{ old('day', $firstCommunion->day) }}"
                                   min="1"
                                   max="31"
                                   required
                    ...
                    <div class="space-y-4 sm:space-y-5">
                        ...
                        <div>
                            ...
                            <textarea name="address"
                                      id="address"
                                      rows="2"
                                      class="w-full px-3 sm:px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent text-sm @error('address') border-red-500 @enderror">{{ old('address', $firstCommunion->address) }}</textarea>
                            @error('address')
                                <p class="text-red-600 text-xs sm:text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text"
                                   name="minister"
                                   id="minister"
                                   value="{{ old('minister', $firstCommunion->minister) }}"
                        ...
                        <div>
                            ...
                            <input type="date"
                                   name="baptismal_date"
                                   id="baptismal_date"
                                   value="{{ old('baptismal_date', $firstCommunion->baptismal_date?->format('Y-m-d')) }}"
                        ...
                        <div>
                            ...
                            <input type="text"
                                   ...
                                   id="baptismal_place"
                                   value="{{ old('baptismal_place', $firstCommunion->baptismal_place) }}"
                ...
                <div class="flex flex-col sm:flex-row gap-3 sm:gap-4">
                    ...
                    <a href="{{ route('secretary.first-communion.show', $firstCommunion) }}"
...
<script>
    function communionForm() {
        // Convert string arrays to object arrays for Alpine
        const namesData = @json(old('names', $firstCommunion->names ?? []));
        const parentsData = @json(old('parents', $firstCommunion->parents ?? []));

        const communicants = namesData.map(name => ({ name }));
        const parents = parentsData.map(parent => ({ name: parent }));

        return {
            communicants: communicants.length > 0 ? communicants : [{ name: '' }],
            parents: parents.length > 0 ? parents : [{ name: '' }],
